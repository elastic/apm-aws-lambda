...
1. To avoid data loss, APM data collected by APM agents needs to be flushed before the execution environment of a lambda function gets frozen.
2. And, flushing APM data must be fast to not impact the response times of lambda function requests.
