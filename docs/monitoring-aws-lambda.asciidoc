...
As described in the section on <<aws-lambda-arch>>, using Elastic APM with AWS Lambda requires to include the Elastic APM AWS Lambda extension and a corresponding Elastic APM agent to the Lambda runtime.
...
These compotents contribute a little to the uncompressed deployment package size of your Lambda function. The Elastic APM AWS Lambda extension and each of the APM agents have an uncompressed package size that is below 10MB. Thus, the overall impact of using Elastic APM on the uncompressed deployment package size of your Lambda function less than 20MB.

=== Performance impact
The <<aws-lambda-arch>> comes with the adavantage that APM data dispatching is decoupled from your function's request processing. 
The Elastic APM AWS Lambda extension flushes APM data to the Elastic backend _after_ your function reponds to the client's request. Thus, it does not affect the latency of the client's request. 
